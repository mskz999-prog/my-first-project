...
> **⚠️ 2026年9月時点で休止中（お蔵入り）です。** 現在の本線は`content_studio/`
> （古着の年代判別Instagramカルーセル生成）に移っています。このプロジェクトのコード・
> データは削除せず残していますが、週次のGitHub Actions自動実行は停止済みです
> （`workflow_dispatch`での手動実行は引き続き可能）。詳細は`CLAUDE.md`参照。
...
1. **データ収集**（`src/scrapers/`）
   - `yahoo_auction.py`: Yahoo!オークションの落札相場ページ（closedsearch）を収集。
     各商品カードに含まれる終了日時（例:「9/3 09:05 終了」、年表記なしなのでスクレイプ
     時刻を基準に前年へのロールオーバーも判定）を`MarketItem.sold_at`として抽出する
     （`_parse_end_datetime`）。メルカリ・BASE等の他ソースは売却日時を取得できていない
     （メルカリは検索結果に日付が出ず、個別ページも公開されていない）ため、日付ベースの
     正確な集計が必要な分析は現状ヤフオクのデータに限られる
     - **半年分バックフィル**：ヤフオクの落札相場検索はそもそも過去180日分しか保持していない
       （それ以上遡ることはヤフオク側の仕様上不可能）。この180日という上限いっぱいまで、
       新しく登場したキーワード（`brand_catalog.yaml`への追加分含む）だけ初回に限って深く
       ページ送りして収集する（`scrape(..., backfill_cutoff=...)`）。日付を見ながら
       180日の壁に達した時点で自動的にページ送りを打ち切るため、固定ページ数を決め打ち
       しない。どのキーワードがバックフィル済みかは`data/trends/yahoo_backfill_state.json`
       に記録し、2回目以降はそのキーワードを`max_pages_per_keyword`の浅い設定（既定1
       ページ）に戻す。`config.yaml`の`sources.yahoo_auction.backfill_keywords_per_run`で
       1回の実行あたり新規バックフィルするキーワード数の上限を設定でき（未設定＝無制限で
       一気に全部バックフィル）、本運用では小さめの値にしてヤフオクへの負荷集中を避ける
       想定
...
2. **正規化・集計**（`src/pipeline/normalize.py`, `collect.py`）
   ...
   - **Levi's週次価格推移データ**（`report_generator._save_levis_weekly_snapshot`）:
     ヤフオクの`sold_at`（実際の落札日時）をもとに、収集したLevi's商品を「実際に売れた週」
     単位でグルーピングし、週ごとの`top_models`/`top_variants`集計結果を
     `data/trends/levis_weekly.jsonl`へ1行ずつ追記する（Levi's×ヤフオクのデータが無い実行は
     スキップ）。日付を取得できないメルカリ等のソースはこの集計には含めない。実行を重ねる
     たびに同じ週のデータが上書きではなく追記されるため、同じ週を後から再度収集した場合は
     最新の行を正とみなす想定。現状はLevi'sのみを対象にした試験的な仕組み。
...
4. **週次自動実行**（`.github/workflows/weekly_report.yml`）※現在は停止中
   - 元々は毎週月曜 9:00 JST（UTC 00:00）にGitHub Actionsで自動実行し、レポートを
     コミット＋ワークフローのアーティファクトとしても保存する仕組み。上記の休止に伴い
     `schedule`トリガーはコメントアウト済み。再開する場合はコメントを外すだけでよい。
