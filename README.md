...
2. **正規化・集計**（`src/pipeline/normalize.py`, `collect.py`）
   ...
   - **era/variantタグ付け**（`collect._fill_missing_tags`）: 「Levi's 501」のような型番だけでは
     3,000円の個体と30万円の個体を区別できないため、501XX・赤耳・ビッグE・66前期/後期・黒カン・
     セルビッジ・デッドストック・日本製/アメリカ製・40s〜00sといった年代/仕様タグをタイトルから
     追加で抽出する（`MarketItem.tags`）。1アイテムに複数のタグが同時に付くこともある（例:
     「赤耳」かつ「ビッグE」）。`report_generator`はこれを`top_variants`（ブランド×モデル×タグ
     単位の件数・価格統計）として`top_models`のさらに一段階細かい粒度で集計する
